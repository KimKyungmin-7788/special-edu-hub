...
/** Google 로고 SVG (공식 색상 그대로, 토큰 예외 허용 — 브랜드 아이콘) */
function GoogleIcon() {
  return (
...
export function AuthForm({ mode }: { mode: Mode }) {
  ...
  async function handleGoogleLogin() {
    ...
    try {
      ...
      // Google OAuth는 리디렉션 방식 — 이 줄 이후는 실행되지 않음
    } catch (err) {
      const message = err instanceof Error ? err.message : "알 수 없는 오류"
      setError(toKoreanError(message))
    ...
    <div className="mx-auto max-w-sm px-4 py-16">
      ...
      <div className="mt-8">
        <button
          ...
          className="flex w-full items-center justify-center gap-3 rounded-md border border-border bg-background px-4 py-2.5 text-sm font-medium hover:bg-muted disabled:opacity-60"
        >
          <GoogleIcon />
